...
public class CameraFollower : Singleton<CameraFollower>
{
    ...
    private Quaternion targerRotate;
    ...
    private void LateUpdate()
    {
        ...
        tf.rotation = Quaternion.Lerp(tf.rotation, targerRotate, Time.deltaTime * moveSpeed);
    ...
    public void ChangeState(State state)
    {
        ...
        targerRotate = offsets[(int)state].localRotation;
        switch (state)
        {
            case State.MainMenu:
                targetOffset = playerOffset;
                targerRotate = Quaternion.Euler(playerRotate);
                break;

            case State.Gameplay:
                targetOffset = offsetMin;
                targerRotate = Quaternion.Euler(gamePlayRotate);
                break;
            default:
                break;
        }
