...
public class UIShop : UICanvas
{
    ...
    public override void CloseDirectly()
    {
        ...
        LevelManager.Ins.player.OnTakeClothesData();
    ...
    internal void SelectBar(ShopBar shopBar)
    {
        ...
        {
            case ShopType.Hat:
                InitShopItems(data.hats.Ts, ref itemEquipped);
                break;
            case ShopType.Pant:
                InitShopItems(data.pants.Ts, ref itemEquipped);
                break;
            case ShopType.Accessory:
                InitShopItems(data.accessories.Ts, ref itemEquipped);
                break;
            case ShopType.Skin:
                InitShopItems(data.skins.Ts, ref itemEquipped);
                break;
            default:
                break;
        }
    ...
    public List<ShopItemData<T>> GetListItem<T>(ShopType shopType) where T : Enum
    {
        switch (shopType)
        {
            case ShopType.Hat: return data.hats.Ts as List<ShopItemData<T>>;
            case ShopType.Pant: return data.pants.Ts as List<ShopItemData<T>>;
            case ShopType.Accessory: return data.accessories.Ts as List<ShopItemData<T>>;
            case ShopType.Skin: return data.skins.Ts as List<ShopItemData<T>>;
            case ShopType.Weapon: return null;
            default: return null;
        }
    }

    private void InitShopItems<T>(List<ShopItemData<T>> items, ref ShopItem itemEquipped) where T : Enum
    {
    ...
    internal void SelectItem(ShopItem item)
    {
        ...
        //check data
        coinTxt.text = currentItem.data.cost.ToString();
        adsTxt.text = currentItem.data.ads.ToString();
    }

    public void BuyButton()
    {
        //TODO: check xem du tien hay ko
    ...
    public void EquipButton()
    {
        ...
        {
            itemEquipped.SetState(ShopItem.State.Bought);
        }
